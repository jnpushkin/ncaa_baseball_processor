...
HISTORICAL_TEAM_LOGOS = {
    ...
    'Aberdeen IronBirds': 'https://www.mlbstatic.com/team-logos/488.svg',  # Moved from affiliated High-A to MLB Draft League in 2026
    ...
    'Down East Wood Ducks': 'https://www.mlbstatic.com/team-logos/485.svg',  # Replaced by Wilson Warbirds in affiliated ball
    'Lynchburg Hillcats': 'https://www.mlbstatic.com/team-logos/481.svg',  # Rebranded as Hill City Howlers in 2026
    'Mississippi Braves': 'https://www.mlbstatic.com/team-logos/430.svg',  # Relocated to Columbus Clingstones
    'Modesto Nuts': 'https://www.mlbstatic.com/team-logos/515.svg',  # Left affiliated MiLB after 2025
...
# MiLB Stadium coordinates:
# venue_key -> (latitude, longitude, team_name, level, team_id, league_name[, display_venue_name])
#
# venue_key is unique for map/checklist identity. display_venue_name is optional
# and is used when multiple active teams share one venue.
MILB_STADIUM_DATA = {
    'ABC Supply Stadium': (42.496763, -89.0429539, 'Beloit Sky Carp', 'A+', 554, 'Midwest League'),
    '7 17 Credit Union Park': (41.07739, -81.52185, 'Akron RubberDucks', 'AA', 402, 'Eastern League'),
    'Erlanger Park': (35.05449, -85.31384, 'Chattanooga Lookouts', 'AA', 498, 'Southern League'),
    ...
    'CarMax Park': (37.5693086, -77.4647425, 'Richmond Flying Squirrels', 'AA', 3410, 'Eastern League'),
    ...
    'City Stadium': (37.3932868, -79.1672407, 'Hill City Howlers', 'A', 481, 'Carolina League'),
    ...
    'DABOS Park': (32.382298, -86.310592, 'Montgomery Biscuits', 'AA', 421, 'Southern League'),
    ...
    'ESL Ballpark': (43.158301, -77.619748, 'Rochester Red Wings', 'AAA', 534, 'International League'),
    'Everett Memorial Stadium': (47.966664, -122.202786, 'Everett AquaSox', 'A+', 403, 'Northwest League'),
    ...
    'George M. Steinbrenner Field': (27.978979741428514, -82.50706715487692, 'Tampa Tarpons', 'A', 587, 'Florida State League'),
    ...
    'Gwinnett Field': (34.0409586, -83.9937909, 'Gwinnett Stripers', 'AAA', 431, 'International League'),
    ...
    'HomeTrust Park': (35.58668, -82.5501, 'Asheville Tourists', 'A+', 573, 'South Atlantic League'),
    'Hops Ballpark': (45.554, -122.9085, 'Hillsboro Hops', 'A+', 419, 'Northwest League'),
    ...
    'L.P. Frans Stadium': (35.747289, -81.3778848, 'Hickory Crawdads', 'A', 448, 'Carolina League'),
    ...
    'Morongo Field': (34.102721, -117.54792, 'Rancho Cucamonga Quakes', 'A', 526, 'California League'),
    ...
    'ONT Field': (34.0184639834586, -117.60331808557788, 'Ontario Tower Buzzers', 'A', 6482, 'California League'),
    ...
    'Roger Dean Chevrolet Stadium (Palm Beach Cardinals)': (26.890957, -80.116365, 'Palm Beach Cardinals', 'A', 279, 'Florida State League', 'Roger Dean Chevrolet Stadium'),
    'Rogers Field at Nat Bailey Stadium': (49.2431966, -123.1054384, 'Vancouver Canadians', 'A+', 435, 'Northwest League'),
    'Route 66 Stadium': (37.21078, -93.2797, 'Springfield Cardinals', 'AA', 440, 'Texas League'),
    ...
    'Wilson Ballpark': (35.7211556, -77.9143011, 'Wilson Warbirds', 'A', 249, 'Carolina League'),
    'Nymeo Field': (39.4016031, -77.4142135, 'Frederick Keys', 'A+', 493, 'South Atlantic League'),
}

# Historic/renamed MiLB teams (from HISTORICAL_TEAM_LOGOS)
HISTORIC_MILB_TEAMS = set(HISTORICAL_TEAM_LOGOS.keys())

# Reverse lookup: current team_name -> league_name
MILB_TEAM_LEAGUES = {
    info[2]: info[5]
    for info in MILB_STADIUM_DATA.values()
    if info[2] not in HISTORIC_MILB_TEAMS
}
...
def get_milb_venue_display_name(venue_key: str, info: tuple) -> str:
    """Return the public venue name for a MiLB stadium entry."""
    return info[6] if len(info) > 6 else venue_key


def iter_milb_stadium_entries(include_historic: bool = True):
    """Yield normalized MiLB stadium/team entries from the static venue map."""
    for venue_key, info in MILB_STADIUM_DATA.items():
        lat, lng, team_name, level_code, team_id, league_name = info[:6]
        if not include_historic and team_name in HISTORIC_MILB_TEAMS:
            continue
        yield {
            'venue_key': venue_key,
            'venue': get_milb_venue_display_name(venue_key, info),
            'lat': lat,
            'lng': lng,
            'team': team_name,
            ...
            'team_id': team_id,
            'league': league_name,
            'historic': team_name in HISTORIC_MILB_TEAMS,
        }
